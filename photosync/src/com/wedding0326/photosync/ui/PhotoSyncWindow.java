...
import com.wedding0326.photosync.ui.explorer.PhotoExplorer;
...
public class PhotoSyncWindow extends ApplicationWindow {
	...
	protected Control createContents(Composite parent) {
		...
		btnAddFolder.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				DirectoryDialog folderdlg = new DirectoryDialog(Display
		...
		listViewer = new ListViewer(grpFolder, SWT.BORDER | SWT.H_SCROLL
				| SWT.V_SCROLL);
		...
		Gallery gallery = new Gallery(tabFolder, SWT.VIRTUAL | SWT.BORDER
	...
	private void redraw(PaintEvent e) {
		if (previewImage != null) {
			...
			double a = (double) previewBounds.height
					/ (double) previewImage.getBounds().height;
			double b = (double) previewBounds.width
					/ (double) previewImage.getBounds().width;
			...
			e.gc.drawImage(previewImage, 0, 0, previewImage.getBounds().width,
					previewImage.getBounds().height, x, 0, scaledWidth,
					scaledHeight);
		}
	}
	...
	protected ToolBarManager createToolBarManager(int style) {
		...
		toolBarManager.add(addPathAction);
	...
	protected void addNewPath() throws IOException {
		...
		DirectoryDialog folderdlg = new DirectoryDialog(Display.getDefault()
				.getActiveShell());
		String selecteddir = folderdlg.open();
		if (selecteddir == null) {
			return;
			...
			MediaBrowser.readMedias(selecteddir, mediaList);
	...
	protected void analysisPhoto() {
		...
		photoExplorer.setCompareResult(result, getStatusLineManager()
				.getProgressMonitor());
		// try {
