package com.wedding0326.photosync.ui.fileBrowser;
...
public class FileBrowser {
    ...
    // FileBrowser.getResourceString("table.Name.title"),
    // FileBrowser.getResourceString("table.Size.title"),
    // FileBrowser.getResourceString("table.Type.title"),
    // FileBrowser.getResourceString("table.Modified.title") };
    ...
    // /**
    // * Returns a string from the resource bundle. We don't want to crash
    // because of a missing String. Returns the key
    // if
    // * not found.
    // */
    //
    ...
    // arguments. If the key is not found,
    // return
    // * the key.
    // */
    ...
    // box.setMessage(getResourceString("dialog.About.description", new Object[]
    ...
    private void createTreeView(Composite parent) {
        ...
        treeScopeLabel = new Label(composite, SWT.NONE);
        treeScopeLabel.setText("details.AllFolders.text");
    ...
    public void createFolderView(Composite parent) {
        display = Display.getDefault();
        // iconCache.initResources(Display.getDefault());
    ...
    private void treeRefreshItem(TreeItem dirItem, boolean forcePopulate) {
        ...
        File[] subFiles = (dir != null) ? FileBrowser.getDirectoryList(dir) : null;
    ...
    private void treeInitFolder(TreeItem item, File folder) {
        ...
        // if (folder != null && folder.list() != null && folder.list().length
        // == 0) {
    ...
    // private File getTargetFile(DropTargetEvent event) {
    // // Determine the target File for the drop
    // TableItem item = table.getItem(table.toControl(new Point(event.x,
    // event.y)));
    // File targetFile = null;
    // if (item == null) {
    ...
    void notifySelectedDirectory(File dir) {
        ...
        /*
         * Tree view: If not already expanded, recursively expands the parents of the specified directory until it is
         * visible.
         */
    ...
    void doDefaultFileAction(File[] files) {
        ...
        if (file.isDirectory()) {
            ...
            if (!Program.launch(fileName)) {
                ...
                // SWT.OK);
                // dialog.setMessage(getResourceString("error.FailedLaunch.message",
                // new Object[] { fileName }));
     ...
     * @param targetFile the File representing the drop target location under inspection, or null if none
     */
    ...
    // progressDialog = new ProgressDialog(shell, (event.detail ==
    // DND.DROP_MOVE) ? ProgressDialog.MOVE :
    // ProgressDialog.COPY);
    ...
    // box.setMessage(getResourceString("dialog.FailedCopy.description", new
    // Object[] { source, dest }));
    // int button = box.open();
    ...
    // processedDropFiles = processedFiles.toArray(new
    // File[processedFiles.size()]);
    // } else {
    // progressDialog.close();
    // progressDialog = null;
    // }
     ...
     */
    // private void dragSourceHandleDragFinished(DragSourceEvent event, String[]
    // sourceNames) {
    // if (sourceNames == null)
    // return;
    ...
    // box.setMessage(getResourceString("dialog.FailedDelete.description", new
    // Object[] { source }));
    // int button = box.open();
    // if (button == SWT.ABORT)
    // i = sourceNames.length;
    ...
    /*
     * This worker updates the table with file information in the background. <p> Implementation notes: <ul> <li> It is
     * designed such that it can be interrupted cleanly. <li> It uses asyncExec() in some places to ensure that SWT
     * Widgets are manipulated in the right thread. Exclusive use of syncExec() would be inappropriate as it would
     * require a pair of context switches between each table update operation. </ul> </p>
     */
    ...
    // @Override
    // public void run() {
    // tableContentsOfLabel.setText(FileBrowser.getResourceString("details.ContentsOf.text",
